...
async function fetchPartner(partnerId: number): Promise<Partner> {
  // Note: partner_status_id is selected only after migration 002_partner_status.sql
  // is applied. Until then the column doesn't exist and PostgREST would error.
  // We do a two-step fetch: first without status, then try to add it.
  const baseSelect = `
    partner_id,
    partner_uid,
    name,
  ...
  // Try to also fetch the status column if migration has been applied
...
export default function PartnerDetail() {
  ...
  // Fetch partner options for CampaignDialog
